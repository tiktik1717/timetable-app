 ...
 */
function isConstraintGroupBlockedAt(
    constraintGroups,
    groupId,
    day,
    hour
...
function getExpectedMeetingUnits(
    meeting,
    teachingUnits,
    approvedExceptions
) {
    return teachingUnits.filter((unit) => {
        if (
            ...
            unit.constraintGroupId !== meeting.id
        ) {
            return false;
        }
...
export function validateSchedule({
    ...
    const constraintGroups =
        schoolData?.constraintGroups || [];

    const dailyHoursByClass =
        schoolData?.dailyHoursByClass || {};
                ...
                if (
                    ...
                    errors.push(
                        createIssue({
                            ...
                            message:
                                `שיבוץ בכיתה ${className} ביום ${day} שעה ${hour} נמצא אחרי שעות הלימוד של הכיתה.`,
                            day,
                            hour,
                            className,
                        })
                ...
                for (const unitId of unitIds) {
                    ...
                    if (!unit) {
                        errors.push(
                            createIssue({
                                type: "unknownUnit",
                                message:
                                    `בתא ${className}, יום ${day}, שעה ${hour} מופיע unitId שאינו קיים: ${unitId}`,
                                day,
                                hour,
                                className,
                                unitId,
                    ...
                    if (
                        ...
                        errors.push(
                            createIssue({
                                type: "wrongClass",
                                message:
                                    `היחידה ${unit.id} שייכת ל-${unit.className} אך שובצה בשורה ${className}.`,
                                day,
                                hour,
                                className,
                                teacherId:
                    ...
                    if (!teacher) {
                        errors.push(
                            createIssue({
                                type: "unknownTeacher",
                                message:
                                    `יחידת ההוראה ${unit.id} מפנה למורה שאינו קיים: ${unit.teacherId}`,
                                day,
                                hour,
                                className,
                                teacherId:
                    ...
                    if (
                        ...
                        errors.push(
                            createIssue({
                                type: "teacherFreeDay",
                                message:
                                    `${teacher.name} שובץ/ה ביום החופשי (${day}).`,
                                day,
                                hour,
                                className,
                                teacherId:
                    ...
                    if (
                        ...
                        errors.push(
                            createIssue({
                                ...
                                message:
                                    `${teacher.name} שובץ/ה ביום ${day} שעה ${hour}, למרות שהשעה חסומה עבורו/ה.`,
                                day,
                                hour,
                                className,
                                teacherId:
                    ...
                    if (
                        ...
                        errors.push(
                            createIssue({
                                type:
                                    "constraintGroupBlocked",
                                message:
                                    `היחידה ${unit.id} שובצה בזמן שחסום לקבוצת השיבוץ שלה.`,
                                day,
                                hour,
                                className,
                                teacherId:
                    ...
                    if (
                        !teacherPlacementsByTime.has(
                            teacherTimeKey
                        )
                    ) {
                        teacherPlacementsByTime.set(
                            teacherTimeKey,
                            []
                        );
                    }

                    teacherPlacementsByTime
                        .get(
                            teacherTimeKey
                        )
                        .push({
    ...
    for (const [
        key,
        placements,
    ] of teacherPlacementsByTime.entries()) {
        ...
        const allPlacementsBelongToSameGroup =
            ...
            placements.every(
                (placement) =>
                    placement.constraintGroupId === groupIds[0]
            );

        if (allPlacementsBelongToSameGroup) {
            const group = getConstraintGroupById(
                constraintGroups,
                groupIds[0]
            );
        ...
        errors.push(
            createIssue({
                ...
                message:
                    `${teacher?.name || teacherId} שובץ/ה ביותר ממקום אחד ביום ${day} שעה ${hour}.`,
                day,
                hour:
                    Number(hour),
                teacherId,
    ...
    for (const unit of teachingUnits) {
        ...
        if (
            ...
            warnings.push(
                createIssue({
                    type:
                        "unscheduledUnitHours",
                    severity:
                        "warning",
                    message:
                        `ליחידה ${unit.id} חסרות ${missing} שעות שיבוץ.`,
                    className:
                        unit.className,
                    teacherId:
                        unit.teacherId,
            ...
            errors.push(
                createIssue({
                    type:
                        "unitOverScheduled",
                    message:
                        `היחידה ${unit.id} שובצה ${scheduledHours} פעמים למרות שנדרשות רק ${requiredHours} שעות.`,
                    className:
                        unit.className,
                    teacherId:
                        unit.teacherId,
    ...
    for (const group of constraintGroups) {
        ...
        const groupUnits = teachingUnits.filter(
            (unit) =>
                unit.constraintGroupId === group.id
        );
        ...
        for (const unit of groupUnits) {
            const placements =
                placementsByUnitId.get(unit.id) || [];

            for (const placement of placements) {
                const key =
                    `${placement.day}|${placement.hour}`;

                if (!placementsBySlot.has(key)) {
                    placementsBySlot.set(
                        key,
                        new Set()
                    );
                }
            ...
            if (
                ...
                errors.push(
                    createIssue({
                        ...
                        message:
                            `קבוצת "${group.name || group.id}" אינה משובצת במלואה יחד ביום ${day} שעה ${hour}.`,
        ...
        for (const [
            ...
            errors.push(
                createIssue({
                    ...
                    message:
                        `בקבוצת "${group.name || group.id}" שובצו מספר יחידות באותו זמן: יום ${day} שעה ${hour}.`,
    ...
    for (const group of constraintGroups) {
        ...
        for (const unit of groupUnits) {
            ...
            for (const placement of placements) {
                const key =
                    `${placement.className}|${placement.day}`;

                if (
                    !placementsByClassAndDay.has(key)
                ) {
                    placementsByClassAndDay.set(
                        key,
                        []
                    );
                }

                placementsByClassAndDay
                    .get(key)
                    .push({
                        unitId: unit.id,
                        hour: placement.hour,
        ...
        for (const [
            key,
            placements,
        ] of placementsByClassAndDay.entries()) {
            ...
            errors.push(
                createIssue({
                    type:
                        "notSameDaySameClassViolation",

                    message:
                        `בקבוצת "${group.name || group.id}" שובצו יותר משעה אחת בכיתה ${className} ביום ${day}.`,
        ...
        if (placementsBySlot.size > 1) {
            errors.push(
                createIssue({
                    ...
                    message:
                        `הישיבה "${meeting.name}" מפוצלת בין יותר מזמן אחד.`,

                    groupId: meeting.id,

                    details: {
                        slots: [...placementsBySlot.entries()]
                            .map(([slotKey, unitIds]) => {
                                const [day, hour] =
                                    slotKey.split("|");
            ...
            if (
                ...
                errors.push(
                    createIssue({
                        ...
                        message:
                            `הישיבה "${meeting.name}" אינה כוללת את כל המשתתפים ביום ${day} שעה ${hour}.`,
            ...
            if (
                ...
                errors.push(
                    createIssue({
                        ...
                        message:
                            `הישיבה "${meeting.name}" שובצה ביום ${day}, שאינו יום מותר עבורה.`,
            ...
            if (
                ...
                errors.push(
                    createIssue({
                        ...
                        message:
                            `הישיבה "${meeting.name}" שובצה בשעה ${hour}, שאינה שעה מותרת עבורה.`,
    ...
    const missingUnits = warnings
        .filter(
            (warning) =>
                warning.type === "unscheduledUnitHours"
        )
        .map((warning) => ({
            ...
            missingHours:
                warning.details?.missingHours || 0,
        }));
    return {
        valid:
            errors.length === 0,
